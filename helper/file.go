...
func UploadFile(file multipart.File, filename string) (string, error) {
	...
	resp, err := cld.Upload.Upload(ctx, file, uploader.UploadParams{PublicID: "authorID-newsID-" + filename})
	...
	asset, err := cld.Admin.Asset(ctx, admin.AssetParams{PublicID: "authorID-newsID-" + filename})
	if err != nil {
